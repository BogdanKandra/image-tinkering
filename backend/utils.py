...
def is_grayscale(image):
    """ Takes an image as parameter and decides whether the image is grayscale or not """
    return len(image.shape) == 2

def is_color(image):
    """ Takes an image as parameter and decides whether the image is color or not """
    return len(image.shape) != 2
...
def merge_channels(channels):
    """ Takes a list of channels as input and outputs the image obtained by merging the channels """
...
def resize_dimension(image, new_height=0, new_width=0, interpolation_method=cv2.INTER_LINEAR):
    """ If one of the dimensions is not given, resizes an image to the specified height (or width), while maintaining
    the original aspect ratio. If given both dimensions, resizes an image to the fixed, specified dimensions

    Arguments:
        image (numPy array) -- the image to be resized

        new_height (int, optional) -- the new height (in pixels) of the resized image; the default value is 0, meaning
        that the image will not be resized by height

        new_width (int, optional) -- same as "new_height", except for the width; if both these arguments are left as
        default, the original image will be returned
    """
...
def resize_percentage(image, percentage=0):
    ...
    Arguments:
        ...
        percentage (int, optional) -- the percentage out of the original image's dimensions to resize to; the default
        value of 0 means that no resizing will be done
    """
    image_h, image_w = image.shape[:2]
    aspect_ratio = image_w / image_h
...
def preprocess_image_dataset(input_directory, new_height, new_width, results_prefix, destination_directory):
    """ Applies several preprocessing operations on the images in the input_directory: if present, the alpha channel is
    stripped; then resizing to the given dimensions is performed; finally, the resulting pixel values are normalized in
    the [0, 255] interval """
...
def pickle_imageset_information(input_directory):
    """ Reads all images present in the input directory; computes, for each image, the average values of the red, green
    and blue channels respectively and places them in a dictionary; this dataset's images dimensions and the dictionary
    are serialized in a pickle having the same name as the input directory """
    files = os.listdir(input_directory)
    averages = {}
